...
def measure(point_up_1, point_down_1, zdf_path, contourrs_point):
    ...
    zero_times = 0
    for i in range(3):
        point_up, point_down = get_point2_incontourrs(point_up_1[i, :], point_down_1[i, :], contourrs_point)
        point_up_x1 = point_up.tolist()
        point_down_x1 = point_down.tolist()
        point_up_x1all = circle_pixel(point_up_x1, radius=1)
        point_down_x1all = circle_pixel(point_down_x1, radius=1)
        a, _ = get_all_xyz(point_up_x1all, zdf_path)
        b, _ = get_all_xyz(point_down_x1all, zdf_path)
        if len(a)*len(b) != 0:
            dis = xyz_dist(a, b)
            print('a:', point_up_x1, ' b: ', point_down_x1, ' dis: ', dis)
            print('all ', point_up_x1all, ' ...', point_down_x1all)
            print('zdf dir: ', zdf_path)
            dis1 = dis1 + dis
        else:
            zero_times += 1
    ...
    if zero_times == 3:
        ...
        zhawa_size = str(dis1/(3 - zero_times))
    return zhawa_size
...
#for i in range(len(img_pre_list)):
    ...
    print('img', img.shape)
    a1 , a2, a3 = get_point_in_img(img1)
    # print('\n', a1.shape, a2.shape)
    # a1, a2, a3 = a1.tolist(), a2.tolist(), a3.tolist()
    # print('\n', len(a1))
    # a1_xyz, a1_xyznan = get_all_xyz(a1, r'D:\liuxz_code\python\mmsegmentation\data\chegou\zdf\330.zdf')
    # print('\n', len(a1_xyz), len(a1_xyznan))
